    ...
    function _handleSwap(
        ...
        if (zeroForOne) {
            ...
            if (state.currentLiquidity > 0) {
                ...
                amount1 = -int256(FullMath.mulDiv(state.amountAfterFee, uint256(sqrtPriceX96), Q96));
            }
        } else {
            // Calculate amounts
            amount0 = -int256(state.amountAfterFee);
            ...
            if (state.currentLiquidity > 0) {
                ...
                // Calculate swap amounts
                amount0 = -int256(FullMath.mulDiv(state.amountAfterFee, Q96, uint256(sqrtPriceX96)));
                amount1 = int256(state.amountAfterFee);
            }
        }
    }
